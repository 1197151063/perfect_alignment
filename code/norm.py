...
import world
from re_model import LightGCN,NRGCF,RGCF
...
from torch_geometric.utils import spmm,dense_to_sparse
import utils
import numpy as np
config = world.config
seed = config['seed']
...
# user_index = dataset.train_edge_index[0].to(device)
# item_index = dataset.train_edge_index[1].to(device)
Recmodel = NRGCF(num_users=num_users,num_items=num_items,edge_index=edge_index)

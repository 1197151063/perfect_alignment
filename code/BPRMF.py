...
Recmodel = MF(num_users=num_users,num_items=num_items,config=config)
Recmodel = Recmodel.to(world.device)
bpr = utils.BPRLoss_MF(Recmodel,config)
Neg_k = 1
w = None
best = ""
best_test=""
...
for epoch in range(world.TRAIN_epochs):
    output_information = Procedure.BPR_train_MF(dataset, Recmodel, bpr)
    cprint("[TEST]")
